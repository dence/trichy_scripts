...
my @kmer_lengths = (31,30,29,28,27,26,25,24,23,22,21,20);
...
#foreach kmer length
	foreach my $kmer_length (@kmer_lengths){
		...
		my $kmer_dir = "single_CC_nucl_k" . $kmer_length; 
		chdir($kmer_dir);
		print 
...
sub make_config_file{
	...
	open(TEMPLATE,"taxonomer.config.template");
	my $kmer_folder="single_CC_nucl_k" . $kmer_length;
	...
	open(NEW_CONFIG, ">$new_config_path");
...
sub substitute_arg{
	...
	my $new_line = $line;
	if($new_line =~ /dbXX/){
		my $currDB="Single_CC_Library.nucl.k" . $kmer_length;
		$new_line =~ s/dbXX/$currDB/;
	}
	elsif($new_line =~ /stiXX/){
		my $currSTI="../Single_CC_Library.nucl.sti";
		$new_line =~ s/stiXX/$currSTI/;
	}
	elsif($new_line =~ /triXX/){
		my $currTRI = "../Single_CC_Library.nucl.tri";
		$new_line =~ s/triXX/$currTRI/;
	}
	elsif($new_line =~ /kcXX/){
		my $currKC = "Single_CC_Library.nucl_taxonomer.k" . $kmer_length . ".kc";
		$new_line =~ s/kcXX/$currKC/;
	}
	elsif($new_line =~ /inXX/){
		my $currIN = "../Single_CC_Library.nucl_taxonomer.fa";
